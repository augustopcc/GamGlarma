...
gamglarma <- Gam_Glarma(
  ...
  phiLags = NULL,
  thetaLags = 1,
  n_spline = 2, auto_spline = T,spline_cols = "NO",
  maxit = 50,
  grad = 1e-6,
  patience = 3,
  trace = F
...
# predict.gamglarma <- function(object,n.ahead = 0, newdata, ...){
#   
  

  
  
  #caso seja informado dados novos, prever para eles
  
  #criar a nova matriz X inputando os dados novos/previstos do autoarima, completar as colunas determinadas como impulso, sen cosseno, passo 
  
  #levar em conta caso haja splines, pois precisa realizar as splines em cima dos valores vindos do autorima ou dos informados que originalmente foram usados em splines
  
  #Multiplica a base de dados de previsão pelos valores vindos da modelagem e calcula pelos valroes do arima.
  ...
  if (is.null(newdata) && n.ahead > 0) {
    ...
    newdata <- data.frame(matrix(ncol = ncol(dados_orig), nrow = n.ahead))
    colnames(newdata) <- colnames(dados_orig)
    
    for (col in colnames(dados_orig)) {
      if (is.numeric(dados_orig[[col]])) {
        modelo_arima <- forecast::auto.arima(dados_orig[[col]])
        newdata[[col]] <- as.numeric(forecast::forecast(modelo_arima, h = n.ahead)$mean)
      }
    }
  ...
  if (length(cols_lineares) > 0) {
    X_new <- cbind(X_new, as.matrix(newdata[, cols_lineares, drop = FALSE]))
  }
  ...
  if (isTRUE(object$ind_spline)) {
    for (var_nome in names(object$Formula_spline)) {
      ...
      matriz_spline_futura <- predict(base_historica, newx = newdata[[var_nome]])
